...
/**
 * \file   ltiGaussianDenoisingGenetics.cpp
 *         Defines the abstract parent class of the application dependent
 *         genetic algorithm interface
 * \author Pablo Alvarado
 * \date   19.08.06
 *
 * $Id: ltiGaussianDenoisingGenetics.cpp,v 1.4 2008-10-14 01:43:11 alvarado Exp $
 */

#include "ltiGaussianDenoisingGenetics.h"
...
namespace lti {
  ...
  _LTI_REGISTER_IN_FACTORY(genetics,gaussianDenoisingGenetics)

  // -------------------------------------------------------------------------
  //   lti::GaussianDenoisingGenetics::parameters
  // -------------------------------------------------------------------------

  
  // default constructor
  gaussianDenoisingGenetics::parameters::parameters()
  ...
  // copy constructor
  gaussianDenoisingGenetics::parameters::parameters(const parameters& other)
    : denoisingGenetics::parameters() {
    copy(other);
  }
  ...
  gaussianDenoisingGenetics::parameters&
  gaussianDenoisingGenetics::parameters::copy(const parameters& other) {
  ...
  gaussianDenoisingGenetics::parameters&
  gaussianDenoisingGenetics::parameters::operator=(const parameters& other) {
    return copy(other);
  }

  // class name
  const std::string& gaussianDenoisingGenetics::parameters::name() const {
    _LTI_RETURN_CLASS_NAME
  }

  // clone method
  gaussianDenoisingGenetics::parameters*
  gaussianDenoisingGenetics::parameters::clone() const {
    return new parameters(*this);
  }

  // new instance
  gaussianDenoisingGenetics::parameters*
  gaussianDenoisingGenetics::parameters::newInstance() const {
   ...
   */
  bool gaussianDenoisingGenetics::parameters::write(ioHandler& handler,
   ...
   */
  bool gaussianDenoisingGenetics::parameters::read(ioHandler& handler,
                                        const bool complete)  {
    bool b = true;
    if (complete) {
      b = handler.readBegin();
  ...
  // -------------------------------------------------------------------------
  //   lti::GaussianDenoisingGenetics
  // -------------------------------------------------------------------------

  /*
   * Default constructor.
   */
  gaussianDenoisingGenetics::gaussianDenoisingGenetics() 
    : denoisingGenetics() {
    
    // set the type of denoiser to be used
    gaussianDenoising gd;
    setDenoiser(gd);
  }
   ...
   */
  const std::string& gaussianDenoisingGenetics::name() const {
    _LTI_RETURN_CLASS_NAME
  }
   ...
   * Clone method.
   */
  gaussianDenoisingGenetics* gaussianDenoisingGenetics::clone() const {
    return new gaussianDenoisingGenetics(*this);
  }

  /*
   * New instance method.
   */
  gaussianDenoisingGenetics* gaussianDenoisingGenetics::newInstance() const {
    return new gaussianDenoisingGenetics();
  }

  // return parameters
  const gaussianDenoisingGenetics::parameters& 
  gaussianDenoisingGenetics::getParameters() const {
   ...
   * - binToDouble()
   */
  bool gaussianDenoisingGenetics::
  chromosomeToPhenotype(const chromosome& genotype,
    ...
    gaussianDenoising::parameters* phen = 
      dynamic_cast<gaussianDenoising::parameters*>(&phenotype);
    ...
    // the next BitsForVariance bits encode the std. deviation
    pos = binToDouble(genotype,pos,BitsForVariance,
                      sqrt(par.varianceInterval.from),
                      sqrt(par.varianceInterval.to),
                      dres);
    phen->variance = sqr(dres); // variance = square of std. dev. 
   ...
   * - doubleToBin()
     */
  gaussianDenoising::parameters* gaussianDenoisingGenetics::
  chromosomeToPhenotype(const chromosome& genotype) const {
    gaussianDenoising::parameters* gdp = new gaussianDenoising::parameters;
  ...
  phenotypeToChromosome(const functor::parameters& phenotype,
    ...
    const gaussianDenoising::parameters* phen = 
      dynamic_cast<const gaussianDenoising::parameters*>(&phenotype);
    ...
    // the variance... well, we rather store the std dev.
    dres = sqrt(phen->variance);    
    pos = doubleToBin(dres,pos,BitsForVariance,
                      sqrt(par.varianceInterval.from),
                      sqrt(par.varianceInterval.to),
                      genotype);
   ...
   */
  int gaussianDenoisingGenetics::getChromosomeSize() const {
    return BitsForBoundaryType + BitsForSize + BitsForVariance;
  }
