 ...
 * \date   29.06.00
 *
 * $Id: ltiHistogram.h,v 1.9 2009-09-13 23:46:59 alvarado Exp $
 */
...
namespace lti {
   ...
   *
   * The lti::histogram template class allows the generation of
   ...
   *
   * The class histogram is a typedef of histogram<double>
   *
   ...
   *  Example:
   *  \code
   *    lti::histogram<float> hist(3,10) // creates a 3D-histogram with 10
  ...
  public:
     ...
     */
    histogram(const int dimensions, const int cells);
     ...
     */
    histogram(const int dimensions,const ivector& cells);
     ...
     *         <code>dim</code>
     */
    inline int getCellsInDimension(const int dimension) const;

    /**
     * get the number of cells per dimension
     */
    inline const ivector& getCellsPerDimension() const;
     ...
     *   lti::histogram<float> myHist(3,10);   // a 3D-histogram with 10 cells/dim. (of type float)
     *   lti::histogram<float>::iterator it;   // an iterator
     *
     ...
     * of entries.
     *
     * You should not use this for histogram<int>, since rounding
     ...
     * Example:
     * \code
     *   lti::histogram<float> hist(1,10);         // 1D-histogram with 10 elements
     *   hist.clear;
     ...
     */
    histogram<T>& copy(const histogram<T>& other);
     ...
     *                 this histogram!
     */
    void detach(histogram<T>& receiver);
    //@}
     ...
     */
    bool equals(const histogram<T>& other) const;
     ...
     * @return true if both histograms have the same elements and same size
     */
    inline bool operator==(const histogram<T>& other) const;
     ...
     */
    bool prettyCloseTo(const histogram<T>& other,
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& apply(T (*function)(T));
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& apply(T (*function)(const T&));
    //@}
     ...
     * left to you!).
     *
     * You should not use this with histogram<int>.
     *
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& emultiply(const histogram<T>& other);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& emultiply(const histogram<T>& first,
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& add(const histogram<T>& other);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& add(const histogram<T>& first,const histogram<T>& second);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& subtract(const histogram<T>& other);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& subtract(const histogram<T>& first,
                           const histogram<T>& second);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& multiply(const T& cst);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& multiply(const histogram<T>& other,const T& cst);
     ...
     * @return a reference to the actual histogram
     */
    inline histogram<T>& operator*=(const T& cst);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& divide(const T& cst);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& divide(const histogram<T>& other,const T& cst);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& add(const histogram<T>& other,const T& cst);
     ...
     * @return a reference to the actual histogram
     */
    histogram<T>& normalize(const bool forceUpdateOfNumEntries=false);
     ...
     */
    inline ivector findIndexOfMaximum() const;
     ...
     * get the index of the biggest element in the histogram
     */
    inline ivector findIndexOfMinimum() const;
  ...
  protected:
     ...
     */
    vector<T> theHistogram_;

    /**
     * number of cells
     */
    ivector theCellsPerDimension_;
  ...
  /**
   * Alias for histogram<double>
   */
  typedef histogram<double> dhistogram;

  /**
   * Alias for histogram<float>
   */
  typedef histogram<float>  fhistogram;
