...
/**
 * \file   ltiSplitImageToYPbPr_inline.h
 *         Split color image to YPbPr
 * \author Pablo Alvarado
 * \date   04.01.2007
 *
 * $Id: ltiSplitImageToYPbPr_inline.h,v 1.1 2007-04-05 22:56:36 alvarado Exp $
 */
...
namespace lti {
  ...
  inline bool splitImageToYPbPr::apply(const rgbaPixel& pixel,
                                       float& c1,
                                       float& c2,
                                       float& c3) const {
    ...
    c2 =  red   *(-0.168736f/255.0f) +
          green *(-0.331264f/255.0f) +
          blue  *  0.5f/255.0f; // U=Pb
    c3 =  red   *  0.5f/255.0f  +
          green *(-0.418688f/255.0f) +
          blue  *(-0.081312f/255.0f); // V=Pr
