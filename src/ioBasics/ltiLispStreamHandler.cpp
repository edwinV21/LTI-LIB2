 ...
 * \date   07.12.2000
 * 
 * $Id: ltiLispStreamHandler.cpp,v 1.7 2013-12-10 10:50:26 alvarado Exp $
 */
...
namespace lti {
  ...
  // get type name
  const std::string& lispStreamHandler::name() const {
  ...
  // clone member
  lispStreamHandler* lispStreamHandler::clone() const {
    return new lispStreamHandler(*this);
  }

  // newInstance member
  lispStreamHandler* lispStreamHandler::newInstance() const {
    return new lispStreamHandler(*this);
  }
  ...
  bool lispStreamHandler::read(char& data)  {
    ...
    if (result) {
      ...
      if (str.find('\'') != std::string::npos) {
        ...
        data = static_cast<char>(atoi(str.c_str())); // asume a numerical representation
      }
    }
  ...
  bool lispStreamHandler::read(byte& data)  {
    ...
    if (result) {
      data = static_cast<byte>(atoi(str.c_str()));
    }
  ...
  bool lispStreamHandler::read(ubyte& data)  {
    ...
    if (result) {
      data = static_cast<ubyte>(atoi(str.c_str()));
    }
  ...
  void lispStreamHandler::getNextLine(std::string& newline) {
    ...
    do {
      c=static_cast<char>(inStream->peek());
      if (c == '\n') {
        c = static_cast<char>(inStream->get());
      }
    } while (!inStream->eof() && (c == '\n'));

    // The standard says that the next method will NOT extract the
    // new line from the stream, and that a \0 will be appended
    // automatically to the buffer.

    // In fact a max of garbageThreshold-1 chars are read, to leave
    // space for the appended zero
    inStream->get(buffer,garbageThreshold,'\n');
    count = static_cast<int>(inStream->gcount()); // number of read chars
    ...
    // last get read the maximum number of chars, but
    // that might even be a high coincidental case!
    if ((count >= garbageThreshold-1) && (inStream->good())) {
      // buffer full and stream alive!
